...
const COLORS: Record<ColorKey, ColorStyle> = {
  red: {
    bgClass: "bg-red-600",
    shadowClass:
      "shadow-[inset_0_2px_4px_rgba(255,255,255,0.4),0_4px_10px_rgba(220,38,38,0.4)]",
  ...
  green: {
    ...
    shadowClass:
      "shadow-[inset_0_2px_4px_rgba(255,255,255,0.4),0_4px_10px_rgba(22,163,74,0.4)]",
  ...
  blue: {
    bgClass: "bg-blue-600",
    shadowClass:
      "shadow-[inset_0_2px_4px_rgba(255,255,255,0.4),0_4px_10px_rgba(37,99,235,0.4)]",
  ...
  yellow: {
    ...
    shadowClass:
      "shadow-[inset_0_2px_4px_rgba(255,255,255,0.4),0_4px_10px_rgba(234,179,8,0.4)]",
  ...
  purple: {
    ...
    shadowClass:
      "shadow-[inset_0_2px_4px_rgba(255,255,255,0.4),0_4px_10px_rgba(147,51,234,0.4)]",
  ...
  orange: {
    ...
    shadowClass:
      "shadow-[inset_0_2px_4px_rgba(255,255,255,0.4),0_4px_10px_rgba(249,115,22,0.4)]",
  ...
  pink: {
    bgClass: "bg-pink-400",
    shadowClass:
      "shadow-[inset_0_2px_4px_rgba(255,255,255,0.4),0_4px_10px_rgba(244,114,182,0.4)]",
  ...
  teal: {
    bgClass: "bg-cyan-500",
    shadowClass:
      "shadow-[inset_0_2px_4px_rgba(255,255,255,0.4),0_4px_10px_rgba(6,182,212,0.4)]",
  ...
  indigo: {
    ...
    shadowClass:
      "shadow-[inset_0_2px_4px_rgba(255,255,255,0.3),0_4px_10px_rgba(55,48,163,0.4)]",
  ...
  lime: {
    bgClass: "bg-lime-400",
    shadowClass:
      "shadow-[inset_0_2px_4px_rgba(255,255,255,0.4),0_4px_10px_rgba(163,230,53,0.4)]",
...
export default function BlockSortGame() {
      ...
      try {
        ...
        if (type === "select") {
          ...
          notes.forEach((freq, idx) => {
            ...
            gainSeq.gain.exponentialRampToValueAtTime(
              0.001,
  ...
  const generatePuzzle = useCallback(
    (diff: Difficulty = difficulty, _lvl: number = level) => {
      ...
      while (!solved && attempts < 200) {
        ...
        for (let i = 0; i < colorsCount; i++) {
          generatedTubes[i] = shuffledBlocks.slice(
            i * capacity,
            (i + 1) * capacity,
          )
        }
  ...
  }, [level, generatePuzzle, difficulty])
    ...
    (currentTubes: string[][]) => {
      ...
      if (won) {
        ...
        localStorage.setItem(
          `block-sort-level-${difficulty}`,
          String(level + 1),
        )
      }
    },
    [difficulty, level, playSound, toast, isTubeCompleteState],
  )

  // 실제 블록 이동 비행 애니메이션 및 상태 반영 함수 (공통 사용)
  const executeMove = useCallback(
    (srcIndex: number, destIndex: number, blockToMove: string) => {
      ...
      if (boardEl && srcEl && destEl) {
        ...
        const startX =
          srcRect.left - boardRect.left + (tubeWidth - blockWidth) / 2
        const startY = srcRect.top - boardRect.top - 24

        const endX =
          destRect.left - boardRect.left + (tubeWidth - blockWidth) / 2
        ...
        setTimeout(() => {
          setTubes((prevTubes) => {
            ...
            setHistory((prevHist) => [
              ...prevHist,
              JSON.parse(JSON.stringify(prevTubes)),
            ])
  ...
  const handleTubeClick = (index: number) => {
    ...
    if (selectedTubeIndex === null) {
      ...
      const candidates: {
        idx: number
        type: "same-pure" | "same-dirty" | "empty"
      }[] = []
      ...
      for (let i = 0; i < tubes.length; i++) {
        ...
        if (targetTube.length === 0) {
          ...
          candidates.push({
            idx: i,
            type: isPure ? "same-pure" : "same-dirty",
          })
        }
      }
            ...
            return (
              <div
                ...
                className={`w-full rounded-xl border transition-all duration-300 ease-out flex items-center justify-center text-[10px] font-bold ${
                  ...
                  shouldTranslate
                    ? "shadow-[0_8px_16px_rgba(0,0,0,0.5)] z-20 scale-105"
                    : ""
                } ${
                  isAnimatingSrc
  ...
  return (
    <main className="min-h-screen bg-neutral-900 text-white font-sans selection:bg-amber-500 selection:text-neutral-900 flex flex-col items-center p-4 md:p-8">
            ...
            <DialogContent className="bg-neutral-800 border-neutral-700 text-white max-w-md">
              <DialogHeader>
                ...
                <DialogDescription className="text-neutral-400 text-sm mt-1">
                  블록 정렬 퍼즐은 모든 블록을 색상별로 분류하여 정리하는 힐링
                  퍼즐 게임입니다.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4 text-neutral-200 text-sm pt-4">
                <p>
                  1. 튜브를 선택(터치/클릭)한 뒤, 다른 튜브를 선택하면 가장 위에
                  위치한 블록이 이동합니다.
                </p>
                <p>
                  2. 블록은 **비어 있는 튜브** 또는 **가장 위의 블록과 색상이
                  동일한 튜브**로만 이동할 수 있습니다.
                </p>
                <p>
                  3. 각 튜브는 정해진 높이(난이도별 3~5개)만큼만 블록을 쌓을 수
                  있습니다.
                </p>
                <p>
                  4. 모든 튜브를 비우거나, **한 가지 색상의 블록으로 가득
                  채우면** 게임에서 승리합니다.
                </p>
                <div className="bg-neutral-900/50 p-3 rounded border border-neutral-700/50 flex flex-col gap-1.5 text-xs text-amber-300">
                  ...
                  <span>
                    - 목적지가 **명확하거나 단 한 곳인 경우** 블록 선택 시
                    자동으로 알아서 이동합니다!
                  </span>
                  <span>
                    - 잘못 움직였다면 **되돌리기(Undo)** 버튼을 눌러 이전 상태로
                    돌릴 수 있습니다.
                  </span>
                  <span>
                    - 도저히 풀 수 없을 때는 **재시작(Restart)**을 누르면 맵이
                    처음 상태로 복원됩니다.
                  </span>
                </div>
              </div>
              ...
              <SelectContent className="bg-neutral-900 border-neutral-700 text-white">
                {Object.entries(DIFFICULTY_CONFIGS).map(([key, config]) => (
                  <SelectItem
                    key={key}
                    value={key}
                    className="focus:bg-neutral-800 focus:text-amber-400 cursor-pointer"
                  >
                    {config.label}
                  </SelectItem>
                ))}
      ...
      <Card
        ref={boardRef}
        className="w-full max-w-4xl bg-neutral-800/30 backdrop-blur-md border border-neutral-800/80 shadow-3xl p-6 md:p-10 flex flex-col items-center justify-center min-h-[450px] relative rounded-3xl overflow-hidden"
      >
        {/* 생성 중 오버레이 */}
        {isGenerating && (
