...
import { Level, Point, Dot } from './levels';
...
export default function GameBoard({
  ...
  const ccw = (a: Point, b: Point, c: Point) => {
    const op = a.x * b.y + b.x * c.y + c.x * a.y - (a.y * b.x + b.y * c.x + c.y * a.x);
    if (op > 0) return 1;
    if (op < 0) return -1;
    return 0;
  };
  ...
  const isSegmentIntersecting = (p1: Point, p2: Point, p3: Point, p4: Point): boolean => {
    ...
    if (ab === 0 && cd === 0) {
      ...
      return minX12 <= maxX34 && minX34 <= maxX12 && minY12 <= maxY34 && minY34 <= maxY12;
    }
  ...
  const getRelativeCoords = (e: React.PointerEvent<HTMLDivElement>): Point | null => {
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    if (!isDragging || !dragColor || activePath.length === 0 || !boardRef.current) return;
    ...
    for (const dot of dots) {
      ...
      if (getDistance(coords, dot.p1) <= DOT_RADIUS || getDistance(coords, dot.p2) <= DOT_RADIUS) {
        forceStopDrag(e);
        onCrash();
        return;
      }
    ...
    Object.entries(paths).forEach(([color, path]) => {
      ...
      for (let i = 0; i < path.length - 1; i++) {
        if (isSegmentIntersecting(newSegment.p1, newSegment.p2, path[i], path[i + 1])) {
  ...
  // SVG 자유 곡선 렌더링
  const renderPath = (points: Point[], color: string, isDraggingThis = false) => {
    ...
    <div className="relative w-full max-w-[480px] mx-auto aspect-square select-none">
      ...
      >
        ...
        >
          ...
          {isDragging && dragColor && renderPathBackground(activePath, dragColor)}
