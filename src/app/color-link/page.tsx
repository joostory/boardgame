...
import { LEVELS, type Point } from "./levels"
...
export default function ColorLinkGame() {
  ...
  const [history, setHistory] = useState<
    { paths: Record<string, Point[]>; completed: string[] }[]
  >([])
  ...
  const handleMove = () => {
    ...
    setHistory((prev) => [
      ...prev,
      { paths: { ...paths }, completed: [...completedColors] },
    ])
  ...
  useEffect(() => {
    const allConnected =
      completedColors.length === dots.length && dots.length > 0
      ...
      <div className="w-full max-w-lg px-4 mt-4 flex flex-col gap-4">
            ...
            <button
              onClick={() =>
                setCurrentLevelIdx((prev) =>
                  Math.min(LEVELS.length - 1, prev + 1),
                )
              }
        ...
        <div className="w-full bg-zinc-850 h-2 rounded-full overflow-hidden border border-zinc-800">
          <div
            ...
            style={{
              width: `${(completedColors.length / dots.length) * 100}%`,
            }}
          />
        </div>
      </div>
          ...
          <div className="bg-zinc-850 border border-zinc-700/50 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-3 text-sm text-zinc-300 mb-6">
              <p className="leading-relaxed">
                1. <strong>동일한 색상 및 알파벳</strong>을 가진 두 개의 점을
                클릭 또는 터치 드래그하여 선으로 연결하세요.
              </p>
              <p className="leading-relaxed">
                2. 선은 격자 제약 없이 <strong>모든 방향으로 자유롭게</strong>{" "}
                그릴 수 있습니다.
              </p>
              <p className="leading-relaxed">
                3. 단, 자신이 그리는 선이{" "}
                <strong>자신의 선, 다른 기존 선, 혹은 다른 색의 점</strong>과
                교차하거나 겹치면 <strong>그 즉시 탈락(Game Over)</strong>
                됩니다!
              </p>
              <p className="leading-relaxed">
                4.{" "}
                ...
                선들을 겹치지 않게 우회하여 모든 색상의 점 쌍을 서로 연결하면
                성공합니다!
              </p>
            </div>
