...
import { MAHJONGG_MAPS, type MahjonggMap } from "@/data/mahjonggData"
import {
  createBoard,
...
export default function MahjonggPage() {
  ...
  const playSound = (
    type: "select" | "match" | "mismatch" | "shuffle" | "win" | "click",
  ) => {
    ...
    try {
      ...
      if (type === "select") {
        ...
        playTone(880.0, 0.15, "sine", 0.05) // A5
      } else if (type === "mismatch") {
        playTone(220.0, 0.25, "triangle") // A3
      } else if (type === "shuffle") {
        playTone(330.0, 0.05, "sawtooth", 0)
        playTone(440.0, 0.05, "sawtooth", 0.05)
        playTone(550.0, 0.05, "sawtooth", 0.1)
      } else if (type === "click") {
        playTone(440.0, 0.04, "sine")
      } else if (type === "win") {
    ...
    if (matches.length > 0) {
      ...
      setTimeout(() => {
        setHintPair((prev) => {
          if (
            prev &&
            prev[0] === randomMatch[0].id &&
            prev[1] === randomMatch[1].id
          ) {
            return null
          }
          return prev
  ...
  const handleTileClick = (tile: TileInstance) => {
    if (!isPlaying || isPaused || gameStatus !== "playing" || tile.isRemoved)
      return
    ...
    <main className="h-screen bg-slate-950 text-slate-100 flex flex-col p-3 overflow-hidden select-none">
      ...
      <header className="w-full flex justify-between items-center pb-2 border-b border-slate-800/80">
        <div className="flex items-center gap-2">
          <Link
            href="/"
            className="p-1 bg-slate-900 rounded hover:bg-slate-800 transition-colors text-slate-400 hover:text-slate-100"
          >
        ...
        {isPlaying && !isPaused && (
          <div className="flex gap-4 items-center bg-slate-900/60 px-3 py-1 rounded border border-slate-800/60 text-xs font-mono">
            ...
            <div>
              ...
              <span
                className={`font-bold ${matchablePairs.length > 0 ? "text-blue-400" : "text-rose-400"}`}
              >
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 bg-slate-900/60 px-2 py-1 rounded text-xs border border-slate-800/60 text-yellow-400">
            ...
            <span>
              {highScores[currentMap.name]
                ? formatTime(highScores[currentMap.name])
                : "--:--"}
            </span>
          </div>
          ...
          {!isPlaying ? (
            ...
            <>
              ...
              <button
                ...
                disabled={
                  isPaused || activeTilesCount === 0 || gameStatus === "won"
                }
                className="bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white py-1 px-2.5 rounded border border-slate-800 transition-all text-xs font-medium disabled:opacity-40 disabled:pointer-events-none"
              >
                섞기
      ...
      >
        {!isPlaying ? (
          <div className="w-full max-w-lg bg-slate-900/35 border border-slate-800/60 rounded-xl flex flex-col justify-center items-center p-6 text-center backdrop-blur-xs">
            ...
            <h2 className="text-lg font-bold text-slate-200 mb-1.5">
              {currentMap.name} 레이아웃
            </h2>
            <p className="text-slate-400 max-w-sm mb-5 text-xs leading-relaxed">
              상단이 막히지 않고 좌우 중 한쪽 면이 열린 동일한 기호의 타일 2개를
              맞추어 제거하세요.
            </p>
            <button
              onClick={handleStartNewGame}
          ...
          <div className="relative w-full h-full flex justify-center items-center overflow-hidden">
            ...
            {gameStatus === "won" && (
              <div className="absolute inset-0 bg-slate-950/95 z-50 flex flex-col items-center justify-center text-center p-4">
                ...
                <p className="text-emerald-400 text-base font-bold mb-4 font-mono">
                  기록: {formatTime(time)}
                </p>
