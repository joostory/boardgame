 ...
 */
export function isOverlapping2D(
  x1: number,
  y1: number,
  x2: number,
  y2: number,
...
export function isOpenTile(
  ...
  const hasTileAbove = activeTiles.some(
    (t) => t.z > tile.z && isOverlapping2D(tile.x, tile.y, t.x, t.y),
  )
...
export function generateTilePool(count: number): TileType[] {
  ...
  // 1. 일반 패 (4장씩 추가)
  const regularTiles = TILE_DEFINITIONS.filter(
    (t) => t.group !== "flower" && t.group !== "season",
  )
  ...
  // 2. 꽃패/계절패 (각 1장씩 추가)
  const specialTiles = TILE_DEFINITIONS.filter(
    (t) => t.group === "flower" || t.group === "season",
  )
