...
export default function FlipCard({ item }: { item: MemoryCard }) {
  ...
  const [revealedValues, _setRevealedValues] = useAtom(revealedCardValuesAtom)
  const [count, setCount] = useAtom(selectedCountAtom)
  const selected = selectedCards.findIndex((it) => it.id === item.id) >= 0
  const revealed = revealedValues.indexOf(item.value) >= 0
  ...
  return (
    <div
      className={cn(
        "rounded-md w-[120px] h-[160px] transition-all flip-card cursor-pointer",
        (selected || revealed) && "flip",
      )}
    ...
    >
      <div className="flip-card-inner rounded-md">
        ...
        <div
          className={cn(
            "flip-card-front rounded-md bg-neutral-600 shadow-md",
            revealed && "bg-blue-500",
          )}
        >
