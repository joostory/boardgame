...
import type { ModooGame, ModooGameExport } from "@/domain/modoo"
...
function StartMenuButton() {
  ...
  function handleExport() {
    const json: ModooGameExport = {
      list: games,
      option: gameOption,
      games: games.map((it) => getGame(it.id)).filter((it) => it != null),
    }
  ...
  function handleImport(e: ChangeEvent<HTMLInputElement>) {
    ...
    if (
      !confirm(
        "게임데이터 가져오기를 하면 이전데이터는 모두 삭제되고 가져온 데이터로 대체됩니다. 계속하시겠습니까?",
      )
    ) {
      return
    }
    ...
    reader.onloadend = () => {
      const json = JSON.parse(reader.result as string) as ModooGameExport
      setGameOption(json.option)
      setGames(json.list)
      json.games.forEach((it: ModooGame) => {
        setGame(it.id, it)
      })
    }
  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          <DropdownMenuItem onClick={handleExport}>내보내기</DropdownMenuItem>
