...
import type { ModooCommand } from "@/domain/modoo"
...
function CommandCard({ command }: { command: ModooCommand }) {
  ...
  return (
    <div className="flex justify-center py-5">
      <div
        className={cn(
          "rounded-md w-[240px] h-[320px] shadow-lg p-4 relative",
          cardBackground,
          animationName,
        )}
      >
        ...
        {command.storable && (
          <Badge className="absolute bottom-3 right-3">보관가능</Badge>
        )}
      </div>
    </div>
  )
...
function RandomCommand() {
  ...
  useEffect(() => {
    ...
    axios
      .get("/modoo/api/command")
      .then((r) => r.data)
      .then((data) => setCommands(data))
      .finally(() => setLoading(false))
  }, [commands, setCommands, selectRandomCommand])
