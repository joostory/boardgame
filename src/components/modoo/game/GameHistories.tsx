...
function Amount({ amount }: { amount: number }) {
  ...
  return <b className={className}>{toNumberFormat(amount)}</b>
}
  ...
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <ArrowUturnLeftIcon className="h-4 w-4 ml-1 cursor-pointer text-foreground/60 hover:text-primary" />
      </AlertDialogTrigger>
      <AlertDialogContent>
        <AlertDialogTitle>기록을 되돌리시겠습니까?</AlertDialogTitle>
    ...
    <Timeline>
      {currentGame.histories.map((it, index) => (
        <TimelineItem
          ...
          title={toTimeFormat(new Date(it.time))}
        >
          <div className="flex gap-1">
            ...
            {currentGame.status !== ModooGameStatus.ENDED && index === 0 && (
              <HistoryRollbackButton history={it} />
            )}
          </div>
        </TimelineItem>
      ))}
