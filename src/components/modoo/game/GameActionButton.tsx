...
import { getTopPlayer, type ModooPlayer, updatePlayer } from "@/domain/modoo"
...
export function SendButton({ player }: { player: ModooPlayer }) {
  ...
  const sendMoney = useAtomCallback(
    useCallback(
      (get, set, to: string, money: number) => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger>
        <ArrowRightEndOnRectangleIcon className="w-8 h-8 p-1 text-blue-400 hover:text-blue-600" />
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
...
export function ReceiveButton({ player }: { player: ModooPlayer }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger>
        <ArrowLeftEndOnRectangleIcon className="w-8 h-8 p-1 text-red-400 hover:text-red-600" />
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
