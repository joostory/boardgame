...
import type { ModooGameMeta } from "@/domain/modoo"
...
function RemoveGameButton({ onRemove }: { onRemove: () => void }) {
  return (
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogTitle>게임기록을 제거하시겠습니까?</AlertDialogTitle>
        <AlertDialogDescription>
          게임기록 제거는 되돌릴 수 없습니다.
        </AlertDialogDescription>
...
export default function GameList() {
  ...
  function handleView(gameMeta: ModooGameMeta) {
    setCurrentGame(getGame(gameMeta.id) ?? undefined)
  }
    ...
    <div className="my-5 mx-5">
      <ul className="w-full">
        {games.toReversed().map((it) => (
          <li
            key={it.id}
            className="flex justify-between rounded-xl py-6 px-6 text-slate-400 transition-all shadow-lg hover:shadow-neutral-900 mb-4"
          >
            <div className="flex min-w-0 w-full">
              <div className="min-w-0 flex-auto grow">
                <p className="text-xl font-semibold leading-6">
                  <span className="mr-1 text-3xl text-yellow-600">
                    {toDateFormat(new Date(it.started))}
                  </span>
                  <span className="text-sm mr-1 text-yellow-700">
                    {toTimeFormat(new Date(it.started))}
                  </span>
              ...
              <div className="shrink-0 flex flex-row items-center gap-x-2">
                ...
                <RemoveGameButton onRemove={() => handleRemove(it)} />
              </div>
            </div>
          </li>
        ))}
