...
export function Timeline({ children }: { children: ReactNode }) {
  return (
    <div className="my-10 mx-5">
      <ul className="px-4 w-full flex flex-col">{children}</ul>
    </div>
  )
}
...
export function TimelineItem({
  ...
  return (
    ...
    >
      ...
      />
      <div className="col-start-2 row-start-2 p-1">{title}</div>
      <div className="col-start-1 row-start-2">
        <CheckCircleIcon className="h-5 w-5" />
      </div>
      <div className="col-start-2 row-start-4 p-1">{children}</div>
      <hr
