...
export default function JigsawGame() {
  ...
  const [containerSize, _setContainerSize] = useState({
    width: 600,
    height: 400,
  })
  ...
  const _getPiecePath = (
    ...
    if (shape.top === 0) path += ` L ${width} 0`
      ...
      path += ` L ${width / 2 - tabSize / 2} 0 L ${width / 2 - tabSize / 2} ${shape.top * -tabSize} L ${width / 2 + tabSize / 2} ${shape.top * -tabSize} L ${width / 2 + tabSize / 2} 0 L ${width} 0`

    // Right
    if (shape.right === 0) path += ` L ${width} ${height}`
    else
      path += ` L ${width} ${height / 2 - tabSize / 2} L ${width + shape.right * tabSize} ${height / 2 - tabSize / 2} L ${width + shape.right * tabSize} ${height / 2 + tabSize / 2} L ${width} ${height / 2 + tabSize / 2} L ${width} ${height}`

    // Bottom
    if (shape.bottom === 0) path += ` L 0 ${height}`
    else
      path += ` L ${width / 2 + tabSize / 2} ${height} L ${width / 2 + tabSize / 2} ${height + shape.bottom * tabSize} L ${width / 2 - tabSize / 2} ${height + shape.bottom * tabSize} L ${width / 2 - tabSize / 2} ${height} L 0 ${height}`

    // Left
    if (shape.left === 0) path += ` L 0 0`
    else
      path += ` L 0 ${height / 2 + tabSize / 2} L ${shape.left * -tabSize} ${height / 2 + tabSize / 2} L ${shape.left * -tabSize} ${height / 2 - tabSize / 2} L 0 ${height / 2 - tabSize / 2} L 0 0`
        ...
        <div className="flex flex-wrap gap-4 justify-between items-center mb-4">
          <div className="flex gap-2 items-center">
            ...
            <Select value={selectedImage} onValueChange={setSelectedImage}>
              ...
              <SelectContent>
                {IMAGES.map((img) => (
                  <SelectItem key={img.id} value={img.src}>
                    {img.label}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div className="flex gap-2 items-center">
            ...
            <Select
              ...
              onValueChange={(val) =>
                setDifficulty(
                  DIFFICULTIES.find((d) => d.id === val) || DIFFICULTIES[0],
                )
              }
            >
              <SelectTrigger className="w-[180px]">
...
function getPiecePathWithOffset(
  width: number,
  height: number,
  shape: Piece["shape"],
  tabSize: number,
