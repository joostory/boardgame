            ...
            >
              {rowJasos.map((jaso, colIndex) => {
                const status = isSubmitted
                  ? guessResults[rowIndex][colIndex]
                  : "empty"

                // 타일 색상 클래스
                let borderBgClass =
                  "border-neutral-800 bg-neutral-900/50 text-neutral-100"
                if (status === "correct") {
                  borderBgClass =
                    "bg-emerald-600 border-emerald-500 text-white shadow-[0_0_12px_rgba(16,185,129,0.3)]"
                } else if (status === "present") {
                  borderBgClass =
                    "bg-amber-600 border-amber-500 text-white shadow-[0_0_12px_rgba(245,158,11,0.3)]"
                } else if (status === "absent") {
                  borderBgClass =
                    "bg-neutral-800 border-neutral-700 text-neutral-500"
                } else if (jaso) {
                  borderBgClass =
                    "border-neutral-600 bg-neutral-900 text-neutral-100 scale-105 transition-transform duration-100"
                }
                  ...
                  <div
                    ...
                    style={{
                      ...
                      animation: isFlipping
                        ? `flip 0.6s ease forward ${delay}ms`
                        : undefined,
                    }}
                  >
                    {jaso}
